...
if (!process.env.REACT_APP_TINK_LINK_PERMANENT_USERS_CLIENT_ID) {
  throw Error(
    "Environment variable `REACT_APP_TINK_LINK_PERMANENT_USERS_CLIENT_ID` is not set."
  );
}
...
app.post("/authorization-code", async (req, res) => {
  const token = await api.fetchClientAccessToken();
  const authorizationCode = await api.fetchAuthorizationCode(
    req.body.user_id,
    token
  );
...
app.get("/user/:userId/credentials", async (req, res) => {
  ...
  const userAccessToken = await api.fetchUserAccessToken(
...
app.get("/payments/requests/:paymentRequestId/transfers", async (req, res) => {
  const accessToken = await api.fetchClientAccessToken();

  const transfers = await api.getTransfers(accessToken, req.params.paymentRequestId);
